...
scanCR:
  ...
  if (BufUsed != CurPos &&
      (pCR = (char _DSPTR *)memchr( LineBuf + CurPos, '\r', BufUsed - CurPos ))
       != NULL)
  {
    *pCR = 0;                   // mark EOL
    ...
    // Skip the LF (if any)
    if (CurPos >= BufUsed)      // if '\r' was the last symbol in the buffer...
    {
      // Append new data in buffer
      AppendLine( res );        // .. shifting _res_ to start of buffer
      // make up for the shifting
      res = LineBuf;

      if (CurPos == BufUsed)    // if didn't read anything more
        return res;
    }
    if (LineBuf[CurPos] == '\n')
      ++CurPos;
